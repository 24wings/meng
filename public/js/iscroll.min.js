...
(function(f, a, e) {
    var h = f.requestAnimationFrame || f.webkitRequestAnimationFrame || f.mozRequestAnimationFrame || f.oRequestAnimationFrame || f.msRequestAnimationFrame || function(i) { f.setTimeout(i, 1000 / 60) };
    var c = (function() {
        var m = {};
        ...
        var k = (function() {
            var r = ["t", "webkitT", "MozT", "msT", "OT"],
                p, q = 0,
                o = r.length;
            for (; q < o; q++) { p = r[q] + "ransform"; if (p in n) { return r[q].substr(0, r[q].length - 1) } }
            return false
        })();

        function l(o) { if (k === false) { return false } if (k === "") { return o } return k + o.charAt(0).toUpperCase() + o.substr(1) }
        ...
        m.prefixPointerEvent = function(o) { return f.MSPointerEvent ? "MSPointer" + o.charAt(7).toUpperCase() + o.substr(8) : o };
        ...
        m.isBadAndroid = (function() { var o = f.navigator.appVersion; if (/Android/.test(o) && !(/Chrome\/\d/.test(o))) { var p = o.match(/Safari\/(\d+.\d)/); if (p && typeof p === "object" && p.length >= 2) { return parseFloat(p[1]) < 535.19 } else { return true } } else { return false } })();
        ...
        m.hasClass = function(p, q) { var o = new RegExp("(^|\\s)" + q + "(\\s|$)"); return o.test(p.className) };
        m.addClass = function(p, q) {
            if (m.hasClass(p, q)) { return }
        ...
        m.preventDefaultException = function(q, p) { for (var o in p) { if (p[o].test(q[o])) { return true } } return false };
        ...
        m.extend(m.ease = {}, {
            quadratic: { style: "cubic-bezier(0.25, 0.46, 0.45, 0.94)", fn: function(o) { return o * (2 - o) } },
            circular: { style: "cubic-bezier(0.1, 0.57, 0.1, 1)", fn: function(o) { return e.sqrt(1 - (--o * o)) } },
            back: { style: "cubic-bezier(0.175, 0.885, 0.32, 1.275)", fn: function(p) { var o = 4; return (p = p - 1) * p * ((o + 1) * p + o) + 1 } },
            bounce: { style: "", fn: function(o) { if ((o /= 1) < (1 / 2.75)) { return 7.5625 * o * o } else { if (o < (2 / 2.75)) { return 7.5625 * (o -= (1.5 / 2.75)) * o + 0.75 } else { if (o < (2.5 / 2.75)) { return 7.5625 * (o -= (2.25 / 2.75)) * o + 0.9375 } else { return 7.5625 * (o -= (2.625 / 2.75)) * o + 0.984375 } } } } },
            elastic: {
                style: "",
                fn: function(o) {
                    var p = 0.22,
    ...
    function g(l, j) {
        ...
        this.options = { resizeScrollbars: true, mouseWheelSpeed: 20, snapThreshold: 0.334, disablePointer: !c.hasPointer, disableTouch: c.hasPointer || !c.hasTouch, disableMouse: c.hasPointer || c.hasTouch, startX: 0, startY: 0, scrollY: true, directionLockThreshold: 5, momentum: true, bounce: true, bounceTime: 600, bounceEasing: "", bounceCallBack: null, preventDefault: true, preventDefaultException: { tagName: /^(INPUT|TEXTAREA|BUTTON|SELECT)$/ }, HWCompositing: true, useTransition: true, useTransform: true, bindToWrapper: typeof f.onmousedown === "undefined", };
    ...
    g.prototype = {
        ...
        _transitionEnd: function(i) {
            if (i.target != this.scroller || !this.isInTransition) { return }
        ...
        _start: function(k) {
            if (!!this.options.luckEvent && this.directionLocked == "v") { k.stopPropagation() }
            this.slideNext = false;
            $(this.scroller).addClass("moving");
            if (c.eventType[k.type] != 1) { var j; if (!k.which) { j = (k.button < 2) ? 0 : ((k.button == 4) ? 1 : 2) } else { j = k.button } if (j !== 0) { return } }
        ...
        _move: function(n) {
            if (!!this.options.luckEvent && this.directionLocked == "v") {
                n.stopPropagation();
                n.preventDefault()
            }
            if (!this.enabled || c.eventType[n.type] !== this.initiated) { return }
            ...
            if (this.options.splitScreenRender) { this.options.splitScreenRender(0) }
            if (!this.directionLocked && !this.options.freeScroll) { if (m > l + this.options.directionLockThreshold) { this.directionLocked = "h" } else { if (l >= m + this.options.directionLockThreshold) { this.directionLocked = "v" } else { this.directionLocked = "n" } } }
            if (this.directionLocked == "h") {
                if (this.options.eventPassthrough == "vertical") { n.preventDefault() } else { if (this.options.eventPassthrough == "horizontal") { this.initiated = false; return } }
                j = 0
            } else {
                if (this.directionLocked == "v") {
                    if (this.options.eventPassthrough == "horizontal") { n.preventDefault() } else { if (this.options.eventPassthrough == "vertical") { this.initiated = false; return } }
                    k = 0
                }
            }
            ...
            if (this.y >= 1 || (e.abs(this.y) - e.abs(this.maxScrollY)) > 1) { this.slideNext = true }
        },
        _end: function(q) {
            if ((!!this.options.luckEvent && this.directionLocked == "v") && !this.options.isAllowClick) {
                q.stopPropagation();
                q.preventDefault()
            }
            if (!this.enabled || c.eventType[q.type] !== this.initiated) { return }
            var p = this;
            var k = setTimeout(function() {
                $(p.scroller).removeClass("moving");
                clearTimeout(k);
                k = null
            }, 300);
            ...
            var s = q.changedTouches ? q.changedTouches[0] : q,
                l, j, o = c.getTime() - this.startTime,
                i = e.round(this.x),
                w = e.round(this.y),
                u = e.abs(i - this.startX),
                t = e.abs(w - this.startY),
                m = 0,
                r = "";
            ...
            this.scrollTo(i, w);
            if (!this.moved) {
                if (this.options.tap) { c.tap(q, this.options.tap) }
                if (this.options.click) { c.click(q) }
                this._execEvent("scrollCancel");
                return
            }
            if (this._events.flick && o < 200 && u < 100 && t < 100) { this._execEvent("flick"); return }
            if (this.options.momentum && o < 300) {
                l = this.hasHorizontalScroll ? c.momentum(this.x, this.startX, o, this.maxScrollX, this.options.bounce ? this.wrapperWidth : 0, this.options.deceleration) : { destination: i, duration: 0 };
                j = this.hasVerticalScroll ? c.momentum(this.y, this.startY, o, this.maxScrollY, this.options.bounce ? this.wrapperHeight : 0, this.options.deceleration) : { destination: w, duration: 0 };
                i = l.destination;
                w = j.destination;
                m = e.max(l.duration, j.duration);
                this.isInTransition = 1;
                if (!!this.options.momentumMax) { var v = $(f).height(); if (e.abs(w - this.startY) > v * 0.5) { if ((w - this.startY) > 0) { w = this.startY + v } else { w = this.startY - v } } }
                if (this.options.splitScreenRender) { this.options.splitScreenRender() }
            }
            if (this.options.snap) {
                var n = this._nearestSnap(i, w);
                this.currentPage = n;
                m = this.options.snapSpeed || e.max(e.max(e.min(e.abs(i - n.x), 1000), e.min(e.abs(w - n.y), 1000)), 300);
                i = n.x;
                w = n.y;
                this.directionX = 0;
                this.directionY = 0;
            ...
            if (i != this.x || w != this.y) {
                if (i > 0 || i < this.maxScrollX || w > 0 || w < this.maxScrollY) { r = c.ease.quadratic }
                this.scrollTo(i, w, m, r);
                return
            }
            this._execEvent("scrollEnd")
        },
        _resize: function() {
            var i = this;
            clearTimeout(this.resizeTimeout);
            this.resizeTimeout = setTimeout(function() { i.refresh() }, this.options.resizePolling)
        },
        resetPosition: function(l) {
            ...
            l = l || 0;
            var j = m;
            if (!this.hasHorizontalScroll || this.x > 0) { i = 0 } else { if (this.x < this.maxScrollX) { i = this.maxScrollX } }
            if (!this.hasVerticalScroll || this.y > 0) { m = 0 } else { if (this.y < this.maxScrollY) { m = this.maxScrollY } }
            if (i == this.x && m == this.y) { return false }
            this.scrollTo(i, m, l, this.options.bounceEasing);
            if (this.options.bounceCallBack && typeof this.options.bounceCallBack === "function") {
                if (this.slideNext) {
                    var k = (j >= 0 ? "up" : "down");
                    this.options.bounceCallBack(k);
                    this.slideNext = false
                }
            }
            return true
        ...
        off: function(k, j) { if (!this._events[k]) { return } var i = this._events[k].indexOf(j); if (i > -1) { this._events[k].splice(i, 1) } },
        _execEvent: function(m) {
            if (!this._events[m]) { return }
            var k = 0,
                j = this._events[m].length;
            if (!j) { return }
            for (; k < j; k++) { this._events[m][k].apply(this, [].slice.call(arguments, 1)) }
        },
        ...
        _transitionTime: function(m) {
            ...
            if (!m && c.isBadAndroid) {
                ...
                var k = this;
                h(function() { if (k.scrollerStyle[j] === "0.0001ms") { k.scrollerStyle[j] = "0s" } })
            }
            if (this.indicators) { for (var l = this.indicators.length; l--;) { this.indicators[l].transitionTime(m) } }
        },
        _transitionTimingFunction: function(k) { this.scrollerStyle[c.style.transitionTimingFunction] = k; if (this.indicators) { for (var j = this.indicators.length; j--;) { this.indicators[j].transitionTimingFunction(k) } } },
        _translate: function(j, l) {
            if (this.options.useTransform) { this.scrollerStyle[c.style.transform] = "translate(" + j + "px," + l + "px)" + this.translateZ } else {
                j = e.round(j);
                l = e.round(l);
            ...
            this.x = j;
            this.y = l;
            if (this.indicators) { for (var k = this.indicators.length; k--;) { this.indicators[k].updatePosition() } }
        },
        ...
        _initIndicators: function() {
            ...
            if (this.options.scrollbars) {
                if (this.options.scrollY) {
                    k = { el: d("v", l, this.options.scrollbars), interactive: l, defaultScrollbars: true, customStyle: n, resize: this.options.resizeScrollbars, shrink: this.options.shrinkScrollbars, fade: this.options.fadeScrollbars, listenX: false };
                    this.wrapper.appendChild(k.el);
                    p.push(k)
                }
                if (this.options.scrollX) {
                    k = { el: d("h", l, this.options.scrollbars), interactive: l, defaultScrollbars: true, customStyle: n, resize: this.options.resizeScrollbars, shrink: this.options.shrinkScrollbars, fade: this.options.fadeScrollbars, listenY: false };
                    this.wrapper.appendChild(k.el);
                    p.push(k)
                }
            }
            if (this.options.indicators) { p = p.concat(this.options.indicators) }
            for (var m = p.length; m--;) { this.indicators.push(new b(this, p[m])) }

            function j(r) { if (o.indicators) { for (var q = o.indicators.length; q--;) { r.call(o.indicators[q]) } } }
            if (this.options.fadeScrollbars) {
                this.on("scrollEnd", function() { j(function() { this.fade() }) });
        ...
        _wheel: function(m) {
            ...
            this.wheelTimeout = setTimeout(function() {
                if (!i.options.snap) { i._execEvent("scrollEnd") }
            ...
            if (this.options.snap) {
                ...
                if (k > 0) { n-- } else { if (k < 0) { n++ } }
                if (j > 0) { l-- } else { if (j < 0) { l++ } }
                this.goToPage(n, l);
                return
            }
            ...
            this.directionX = k > 0 ? -1 : k < 0 ? 1 : 0;
            this.directionY = j > 0 ? -1 : j < 0 ? 1 : 0;
            if (n > 0) { n = 0 } else { if (n < this.maxScrollX) { n = this.maxScrollX } }
            if (l > 0) { l = 0 } else { if (l < this.maxScrollY) { l = this.maxScrollY } }
            this.scrollTo(n, l, 0)
        },
        _initSnap: function() {
            this.currentPage = {};
                ...
                if (this.options.snap === true) {
                    ...
                    while (u > -this.scrollerWidth) {
                        ...
                        while (t > -this.scrollerHeight) {
                            this.pages[s][q] = { x: e.max(u, this.maxScrollX), y: e.max(t, this.maxScrollY), width: w, height: v, cx: u - r, cy: t - p };
                            t -= v;
                            q++
                        }
                        u -= w;
        ...
        _nearestSnap: function(k, p) {
            ...
            if (k > 0) { k = 0 } else { if (k < this.maxScrollX) { k = this.maxScrollX } }
            if (p > 0) { p = 0 } else { if (p < this.maxScrollY) { p = this.maxScrollY } }
            for (; o < n; o++) { if (k >= this.pages[o][0].cx) { k = this.pages[o][0].x; break } }
            n = this.pages[o].length;
            for (; j < n; j++) { if (p >= this.pages[0][j].cy) { p = this.pages[0][j].y; break } }
            if (o == this.currentPage.pageX) {
                o += this.directionX;
                if (o < 0) { o = 0 } else { if (o >= this.pages.length) { o = this.pages.length - 1 } }
                k = this.pages[o][0].x
            }
        ...
        goToPage: function(i, n, j, m) {
            ...
            if (i >= this.pages.length) { i = this.pages.length - 1 } else { if (i < 0) { i = 0 } }
            if (n >= this.pages[i].length) { n = this.pages[i].length - 1 } else { if (n < 0) { n = 0 } }
        ...
        _initKeys: function(l) {
            ...
            var j;
            if (typeof this.options.keyBindings == "object") { for (j in this.options.keyBindings) { if (typeof this.options.keyBindings[j] == "string") { this.options.keyBindings[j] = this.options.keyBindings[j].toUpperCase().charCodeAt(0) } } } else { this.options.keyBindings = {} }
        ...
        _key: function(n) {
            ...
            switch (n.keyCode) {
                case this.options.keyBindings.pageUp:
                    if (this.hasHorizontalScroll && !this.hasVerticalScroll) { o += i ? 1 : this.wrapperWidth } else { m += i ? 1 : this.wrapperHeight }
                    break;
        ...
        handleEvent: function(i) {
            switch (i.type) {
                ...
                case "keydown":
                    ...
                    break;
                case "click":
                    if (this.enabled && !i._constructed) { i.preventDefault() }
                    break
            }
        }
    };
    ...
    function b(j, m) {
        ...
        for (var n in m) { this.options[n] = m[n] }
        this.sizeRatioX = 1;
        this.sizeRatioY = 1;
        this.maxPosX = 0;
        this.maxPosY = 0;
        ...
        if (this.options.fade) {
            ...
            var l = this;
            if (c.isBadAndroid) { h(function() { if (l.wrapperStyle[k] === "0.0001ms") { l.wrapperStyle[k] = "0s" } }) }
    ...
    b.prototype = {
        ...
        transitionTime: function(k) {
            ...
            if (!k && c.isBadAndroid) {
                ...
                var j = this;
                h(function() { if (j.indicatorStyle[i] === "0.0001ms") { j.indicatorStyle[i] = "0s" } })
            }
        },
        ...
        updatePosition: function() {
            ...
            this.x = i;
            this.y = j;
            if (this.scroller.options.useTransform) { this.indicatorStyle[c.style.transform] = "translate(" + i + "px," + j + "px)" + this.scroller.translateZ } else {
                this.indicatorStyle.left = i + "px";
                this.indicatorStyle.top = j + "px"
            }
        },
        _pos: function(i, j) {
            if (i < 0) { i = 0 } else { if (i > this.maxPosX) { i = this.maxPosX } }
            if (j < 0) { j = 0 } else { if (j > this.maxPosY) { j = this.maxPosY } }
    ...
    g.utils = c;
    if (typeof module != "undefined" && module.exports) { module.exports = g } else { if (typeof define == "function" && define.amd) { define(function() { return g }) } else { f.IScroll = g } }

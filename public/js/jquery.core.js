(function() {
    var b = function() {
        var e = /\?/;
        var d = { type: "get", url: "", data: "", error: function() {}, success: function() {} };
        var c = function(g) {
            var f = null;
            g.type = g.type || d.type;
            g.url = g.url || d.url;
            g.data = g.data || d.data;
            g.error = g.error || d.error;
            g.success = g.success || d.success;
            if (window.XMLHttpRequest) { f = new XMLHttpRequest() } else { return }
            if (g.type != "post") {
                g.url += (this.r.test(g.url) ? "&" : "?") + g.data;
                f.open("GET", g.url, true);
                f.onreadystatechange = function() { if (f.readyState == 4 && f.status == 200) { g.success(f.responseText) } else { if (g.error) { g.error() } } };
                f.send()
            } else {
                f.open("POST", g.url, true);
                f.setRequestHeader("Content-type", "application/x-www-form-urlencoded; charset=UTF-8");
                f.onreadystatechange = function() { if (f.readyState == 4 && f.status == 200) { g.success(f.responseText) } else { g.error() } };
                f.send(g.data)
            }
        };
        return { ajax: function(f) { try { c(f) } catch (g) {} } }
    }();
    var a = [];
    window.onerror = function(d, c, e) {
        if (e < 1 || typeof d != "string" || d.length < 1) { return }
        if (/WeixinJSBridge|_WXJS/.test(d)) { return }
        var g = "Error:" + d + ";Line:" + e + ";Url:" + c + ";UserAgent:" + navigator.userAgent;
        var f = "Error:" + d + "\nLine:" + e + "\nUrl:" + c + "\n";
        var k = function(i) { return typeof i === "undefined" ? "" : encodeURIComponent(i) };
        var l = true;
        var j = { m: d, u: c, l: e };
        for (var h = 0; h < a.length; h++) { if (a[h].m == j.m && a[h].u == j.u && a[h].l == j.l) { l = false; break } }
        if (l) {
            a.push(j);
            b.ajax({ type: "post", url: "ajax/logJsErr_h.jsp?cmd=jsErr", data: "msg=" + k(g) })
        }
        console.log(f)
    }
})();
...
(function(b, a) { if (typeof module === "object" && typeof module.exports === "object") { module.exports = b.document ? a(b, true) : function(c) { if (!c.document) { throw new Error("jQuery requires a window with a document") } return a(c) } } else { a(b) } }(typeof window !== "undefined" ? window : this, function(window, noGlobal) {
    ...
    jQuery.extend({
        ...
        isPlainObject: function(obj) { if (jQuery.type(obj) !== "object" || obj.nodeType || jQuery.isWindow(obj)) { return false } if (obj.constructor && !hasOwn.call(obj.constructor.prototype, "isPrototypeOf")) { return false } return true },
        ...
        (function(window) {
            ...
            setDocument = Sizzle.setDocument = function(node) {
                ...
                sortOrder = hasCompare ? function(a, b) {
                    ...
                    if (compare & 1 || (!support.sortDetached && b.compareDocumentPosition(a) === compare)) { if (a === doc || a.ownerDocument === preferredDoc && contains(preferredDoc, a)) { return -1 } if (b === doc || b.ownerDocument === preferredDoc && contains(preferredDoc, b)) { return 1 } return sortInput ? (indexOf(sortInput, a) - indexOf(sortInput, b)) : 0 }
            ...
            Expr = Sizzle.selectors = {
                ...
                preFilter: {
                    ...
                    PSEUDO: function(match) {
                        ...
                        if (match[3]) { match[2] = match[4] || match[5] || "" } else {
            ...
            function matcherFromGroupMatchers(elementMatchers, setMatchers) {
                var bySet = setMatchers.length > 0,
                    ...
                    superMatcher = function(seed, context, xml, results, outermost) {
                        ...
                        for (; i !== len && (elem = elems[i]) != null; i++) { if (byElement && elem) { j = 0; while ((matcher = elementMatchers[j++])) { if (matcher(elem, context, xml)) { results.push(elem); break } } if (outermost) { dirruns = dirrunsUnique } } if (bySet) { if ((elem = !matcher && elem)) { matchedCount-- } if (seed) { unmatched.push(elem) } } }
                        matchedCount += i;
    ...
    jQuery.extend({
        ...
        sibling: function(n, elem) { var matched = []; for (; n; n = n.nextSibling) { if (n.nodeType === 1 && n !== elem) { matched.push(n) } } return matched }
    });
    jQuery.fn.extend({
        has: function(target) {
        ...
        index: function(elem) { if (!elem) { return (this[0] && this[0].parentNode) ? this.first().prevAll().length : -1 } if (typeof elem === "string") { return indexOf.call(jQuery(elem), this[0]) } return indexOf.call(this, elem.jquery ? elem[0] : elem) },
            ...
            self = {
                add: function() {
                    if (list) {
                        ...
                        if (firing) { firingLength = list.length } else {
                            if (memory) {
                                firingStart = start;
                                fire(memory)
                            }
    ...
    var access = jQuery.access = function(elems, fn, key, value, chainable, emptyGet, raw) {
        var i = 0,
            len = elems.length,
            bulk = key == null;
        if (jQuery.type(key) === "object") { chainable = true; for (i in key) { jQuery.access(elems, fn, i, key[i], true, emptyGet, raw) } } else {
            if (value !== undefined) {
                chainable = true;
    ...
    jQuery.event = {
        ...
        trigger: function(event, data, elem, onlyHandlers) {
            ...
            while ((cur = eventPath[i++]) && !event.isPropagationStopped()) {
                ...
                if (handle && handle.apply && jQuery.acceptData(cur)) { event.result = handle.apply(cur, data); if (event.result === false) { event.preventDefault() } }
            }
            event.type = type;
        ...
        fixHooks: {},
        keyHooks: { props: "char charCode key keyCode".split(" "), filter: function(event, original) { if (event.which == null) { event.which = original.charCode != null ? original.charCode : original.keyCode } return event } },
        mouseHooks: {
        ...
        simulate: function(type, elem, event, bubble) { var e = jQuery.extend(new jQuery.Event(), event, { type: type, isSimulated: true, originalEvent: {} }); if (bubble) { jQuery.event.trigger(e, null, elem) } else { jQuery.event.dispatch.call(elem, e) } if (e.isDefaultPrevented()) { event.preventDefault() } }
    };
    ...
    jQuery.extend({
        ...
        cleanData: function(elems) {
            ...
            for (;
                ...
                if (jQuery.acceptData(elem)) { key = elem[data_priv.expando]; if (key && (data = data_priv.cache[key])) { if (data.events) { for (type in data.events) { if (special[type]) { jQuery.event.remove(elem, type) } else { jQuery.removeEvent(elem, type, data.handle) } } } if (data_priv.cache[key]) { delete data_priv.cache[key] } } }
            ...
            if (l) {
                ...
                if (first) {
                    ...
                    if (hasScripts) {
                        ...
                        for (i = 0; i < hasScripts; i++) { node = scripts[i]; if (rscriptType.test(node.type || "") && !data_priv.access(node, "globalEval") && jQuery.contains(doc, node)) { if (node.src) { if (jQuery._evalUrl) { jQuery._evalUrl(node.src) } } else { jQuery.globalEval(node.textContent.replace(rcleanScript, "")) } } }
                    }
                }
            }
            return this
    ...
    var getStyles = function(elem) { if (elem.ownerDocument.defaultView.opener) { return elem.ownerDocument.defaultView.getComputedStyle(elem, null) } return window.getComputedStyle(elem, null) };
    ...
    function addGetHookIf(conditionFn, hookFn) { return { get: function() { if (conditionFn()) { delete this.get; return } return (this.get = hookFn).apply(this, arguments) } } }(function() {
        ...
        if (window.getComputedStyle) {
            jQuery.extend(support, {
                pixelPosition: function() { computePixelPositionAndBoxSizingReliable(); return pixelPositionVal },
                boxSizingReliable: function() { if (boxSizingReliableVal == null) { computePixelPositionAndBoxSizingReliable() } return boxSizingReliableVal },
    ...
    function showHide(elements, show) {
        ...
        for (; index < length; index++) {
            ...
            if (show) { if (!values[index] && display === "none") { elem.style.display = "" } if (elem.style.display === "" && isHidden(elem)) { values[index] = data_priv.access(elem, "olddisplay", defaultDisplay(elem.nodeName)) } } else { hidden = isHidden(elem); if (display !== "none" || !hidden) { data_priv.set(elem, "olddisplay", hidden ? display : jQuery.css(elem, "display")) } }
        }
    ...
    jQuery.fn.extend({
        ...
        animate: function(prop, speed, easing, callback) {
            var empty = jQuery.isEmptyObject(prop),
                ...
                doAnimation = function() { var anim = Animation(this, jQuery.extend({}, prop), optall); if (empty || data_priv.get(this, "finish")) { anim.stop(true) } };
    ...
    boolHook = { set: function(elem, value, name) { if (value === false) { jQuery.removeAttr(elem, name) } else { elem.setAttribute(name, name) } return name } };
    ...
    jQuery.extend({
        valHooks: {
            option: { get: function(elem) { var val = jQuery.find.attr(elem, "value"); return val != null ? val : jQuery.trim(jQuery.text(elem)) } },
            select: {
                get: function(elem) {
                ...
                set: function(elem, value) {
                    var optionSet, option, options = elem.options,
                        ...
                        i = options.length;
                    while (i--) { option = options[i]; if ((option.selected = jQuery.inArray(option.value, values) >= 0)) { optionSet = true } }
    ...
    jQuery.each(["radio", "checkbox"], function() { jQuery.valHooks[this] = { set: function(elem, value) { if (jQuery.isArray(value)) { return (elem.checked = jQuery.inArray(jQuery(elem).val(), value) >= 0) } } }; if (!support.checkOn) { jQuery.valHooks[this].get = function(elem) { return elem.getAttribute("value") === null ? "on" : elem.value } } });
    ...
    jQuery.extend({
        ...
        ajax: function(url, options) {
            ...
            var transport, cacheURL, responseHeadersString, responseHeaders, timeoutTimer, parts, fireGlobals, i, s = jQuery.ajaxSetup({}, options),
                ...
                jqXHR = {
                    ...
                    statusCode: function(map) { var code; if (map) { if (state < 2) { for (code in map) { statusCode[code] = [statusCode[code], map[code]] } } else { jqXHR.always(map[jqXHR.status]) } } return this },
            ...
            if (typeof $("#_TOKEN").attr("value") === "string") { s.url += (rquery.test(s.url) ? "&" : "?") + "_TOKEN=" + $("#_TOKEN").attr("value") }
        ...
        getCacheScript: function(url, callback) { return jQuery.ajax({ url: url, type: "get", dataType: "script", data: undefined, success: callback, cache: true }) }
    });
    ...
    jQuery.fn.extend({
        wrapAll: function(html) {
            ...
            if (this[0]) {
                ...
                wrap.map(function() { var elem = this; while (elem.firstElementChild) { elem = elem.firstElementChild } return elem }).append(this)
            }
            return this
        },
    ...
    jQuery.each({ scrollLeft: "pageXOffset", scrollTop: "pageYOffset" }, function(method, prop) {
        ...
        jQuery.fn[method] = function(val) { return access(this, function(elem, method, val) { var win = getWindow(elem); if (val === undefined) { return win ? win[prop] : elem[method] } if (win) { win.scrollTo(!top ? val : window.pageXOffset, top ? val : window.pageYOffset) } else { elem[method] = val } }, method, val, arguments.length, null) }
    });
    ...
    jQuery.fn.tap = function(callback) {
        if ("ontouchstart" in window) {
            if (!this.length) { return } else {
                this.each(function() {
                    if (!callback) { $(this).trigger("touchend"); return }
                    var oldX, oldY, newX, newY, isMoved;
                    this.addEventListener("touchstart", function(event) {
                        isMoved = false;
                        oldX = event.targetTouches[0].screenX;
                        oldY = event.targetTouches[0].screenY
                    });
                    this.addEventListener("touchmove", function(event) {
                        if (isMoved) { return }
                        newX = event.targetTouches[0].screenX;
                        newY = event.targetTouches[0].screenY;
                        (newX - oldX != 0 || newY - oldY != 0) && (isMoved = true)
                    });
                    $(this).on("touchend", function(event) {
                        !isMoved && callback.call(this, event);
                        isMoved = false
                    })
                })
            }
        } else { callback ? this.click(callback) : this.click() }
        return this
    };
